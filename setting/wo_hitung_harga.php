...
if($produk==1){
	...
	$cek = mysqli_query($config,"select * from harga_jual where hj_produk='$produk' and hj_mesin='$mesin' and hj_bahan='$bahan' and hj_sisi='$sisi' and hj_finishing='$finishing' and hj_sisi_finishing='$sisi_finishing' and hj_potong='$potong'");
	if(mysqli_num_rows($cek) > 0){
		$c = mysqli_fetch_assoc($cek);
		echo $c['hj_harga'];	
	}else{
