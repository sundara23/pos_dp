		...
		<div class="row">
			...
			<div class="row">
												...
												while($d=mysqli_fetch_array($data)){
													...
													<tr>																										
														...
														<td>
															<?php  
															$id_mesin = $d['mesin_id'];
															$jumlah = mysqli_query($config,"select * from orderan,harga_jual,invoice where order_hj=hj_id and hj_mesin='$id_mesin' and order_invoice=invoice_id and date(invoice_tgl) >= '$from' and date(invoice_tgl) <= '$to'");
														...
														<td>
															...
															$xxx = 0;
															$id_mesin = $d['mesin_id'];
															$jumlah = mysqli_query($config,"select sum(order_harga_sub_total) as total from orderan,harga_jual,invoice where order_hj=hj_id and hj_mesin='$id_mesin' and order_invoice=invoice_id and date(invoice_tgl) >= '$from' and date(invoice_tgl) <= '$to'");
															$j = mysqli_fetch_assoc($jumlah);
															if($j['total']>0){
																$xxx+=$j['total'];
				...
				<div class="col-md-6">
					<div class="panel">
						<div class="panel-heading">
							<h5><i id="icon-chart"></i> Chart Bahan Lembar</h5>
						</div>
						<div class="panel-body">
	...
	<script type="text/javascript">
		...
		Highcharts.chart('chart-omset-per-mesin', {
			...
			series: [{
				...
				<?php	
				$xxxx = 0;										
				$data = mysqli_query($config,"select * from invoice where date(invoice_tgl) >= '$from' and date(invoice_tgl)<='$to'");		
				while($d=mysqli_fetch_array($data)){
					?>

					<?php  
					$id_i = $d['invoice_tgl'];
					$jumlah = mysqli_query($config,"select * from invoice where invoice_tgl='$id_i' order by date(invoice_tgl) desc");
					$j = mysqli_num_rows($jumlah);						
					?>																															
