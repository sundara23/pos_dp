...
<div class="invoice-box">
    <table cellpadding="0" cellspacing="0">
        ...
        <?php
        $no = 1;
        $data = mysqli_query($config,"select * from orderan_atk,produk_atk where orderan_atk.kd_produk=produk_atk.kd_produk and  order_atk_inv='$id_invoice'");
        while($d=mysqli_fetch_array($data)){
        ?>
        <tr class="item">
            <td><?php echo ucfirst($d['nama_produk']).' ('.$d['order_qty'].')'; ?></td>
            <td>Rp. <?php echo number_format($d['order_harga_subtotal']) ?></td>
        </tr>
        <?php }
        if($k['trx_diskon'] > 0){
        ?>
    ...
    <a href="atk_detail_print.php?id=<?php echo $id_invoice; ?>" class="btn btn-danger" id="download_inv" ><i class="fas fa-print"></i> Cetak Invoice</a>
